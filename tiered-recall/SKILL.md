...
name: tiered-recall
version: 1.2.0
description: 分层回忆系统 - 解决上下文长度限制，保持项目延续性。每次新session自动加载核心记忆+最近日志+活跃项目，支持手动深度回忆。v1.2.0修复索引膨胀bug：索引只存文件路径+行号，不存摘要，L3从34k降至3k token（-91%）。
license: MIT
author: davidme6
...
changelog: "v1.2.0: 修复L3索引token膨胀bug，slim index格式，-91% token。v1.1.0: 增加10字内极致摘要（已废弃，有膨胀问题）。v1.0.3: 加标题。v1.0.0: 初始版本"
---
...
**总预算：~22k token，约占总上下文的11%**

> ⚠️ v1.1.0 的索引含摘要，实际消耗达34k（超标33倍）。v1.2.0 修复：索引只存文件路径+行号，无摘要，实测3k以内。
...
def build_index(memory_dir):
    """扫描日志，提取主题和关键词 - slim index格式，只存路径+行号"""
    ...
    for file in sorted(memory_dir.glob("*.md"), reverse=True):
        ...
        # 识别主题 - 只存 "filename:lines"，不存摘要
        for section in sections:
            topic = classify_topic(section["title"])
            if topic:
                ref = f"{file.name}:{section['lines']}"   # e.g. "2026-03-23.md:100-200"
                index["topics"].setdefault(topic, []).append(ref)
    
    return index

# ✅ 正确输出格式（slim）：
# {"topics": {"游戏开发": ["2026-03-23.md:100-200", "2026-03-24.md:50-150"]}}
# 每条约30字符，170条 ≈ 5KB ≈ 2.5k token
...
| 技能 | 协作方式 |
|------|----------|
| `jarvis-core` | 启动仪式中调用，作为记忆加载层 |
| `self-improving` | 共享记忆结构，增量更新 |
...
### v1.2.0 (2026-04-23)
- 🐛 修复L3索引token膨胀bug（v1.1.0实测34k，超标33倍）
- ✅ slim index格式：只存 "filename:lines"，不存摘要
- 📉 L3从34k → 3k token（-91%），总上下文从50k → 19k（-62%）
- 🔧 更新build_index()实现，废弃summarize()调用
- 📊 L3预算上限更新为3k（原1k过于乐观）

### v1.1.0
- 增加10字摘要（已废弃：导致token膨胀，见v1.2.0修复）
